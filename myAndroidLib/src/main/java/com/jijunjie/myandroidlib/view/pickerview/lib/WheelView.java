package com.jijunjie.myandroidlib.view.pickerview.lib;
...
import com.jijunjie.myandroidlib.view.pickerview.adapter.WheelAdapter;
import com.jijunjie.myandroidlib.view.pickerview.listener.OnItemSelectedListener;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        while (counter < itemsVisible) {
            ...
            if (angle >= 90F || angle <= -90F) {
                ...
                else if (contentText.length() >= 5 && contentText.length() < 8) setTextSize(14);
                else if (contentText.length() > 8) setTextSize(10);
                else if (contentText.length() > 11) setTextSize(8);
                //计算开始绘制的位置
